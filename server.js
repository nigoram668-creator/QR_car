...
function auth(req, res, next) {
  ...
  if (!session) {
    return res.status(401).json({ error: "Login va parol kerak" });
  }

  const car = cars.find(c => c.qr_id === session.qr_id);

  if (!car) {
    sessions.delete(token);
    return res.status(401).json({ error: "Profil topilmadi" });
  }
...
function nextQRId() {
  ...
  while (
    cars.some(
      c => c.qr_id === QRCAR${String(n).padStart(4, "0")}
    )
  ) {
    n++;
  }

  return QRCAR${String(n).padStart(4, "0")};
}
...
app.get("/api/admin/qr-list", (req, res) => {
  res.json(
    cars.map(c => ({
      qr_id: c.qr_id,
      claimed: !!c.claimed,
...
app.get("/api/car/:id", (req, res) => {
  const car = cars.find(c => c.qr_id === req.params.id);
...
app.post("/api/register", (req, res) => {
  ...
  const userLogin = String(username  login  "").trim();
  const userPassword = String(password || "");

  if (!qr_id) {
    return res.status(400).json({ error: "QR ID kerak" });
  }
  ...
  if (!userLogin) {
    return res.status(400).json({ error: "Login kiriting" });
  }
  ...
  const car = cars.find(c => c.qr_id === qr_id);
  ...
  if (car.claimed) {
    return res.status(409).json({
      error: "Bu QR allaqachon ro'yxatdan o'tgan"
    });
  }
if (
    cars.some(
      c => c.claimed && c.username === userLogin
    )
  ) {
    return res.status(409).json({
  ...
  car.car_number = String(car_number).trim();
  car.car_model = String(car_model).trim();
  car.info = String(info).trim();
  car.username = userLogin;
  car.password_hash = hashPassword(userPassword);
...
app.post("/api/login", (req, res) => {
  ...
  const car = cars.find(
    c => c.claimed && c.username === username
  );

  if (
    !car ||
    car.password_hash !== hashPassword(password)
  ) {
...
/* PROFILNI TAHRIRLASH UCHUN LOGIN */
...
app.put("/api/me", auth, (req, res) => {
  ...
  if (name !== undefined)
    req.car.name = String(name).trim();

  if (phone !== undefined)
    req.car.phone = String(phone).trim();

  if (car_number !== undefined)
    req.car.car_number = String(car_number).trim();

  if (car_model !== undefined)
    req.car.car_model = String(car_model).trim();

  if (info !== undefined)
    req.car.info = String(info).trim();
...
<p>
        📞
        <a href="tel:${escapeAttr(car.phone)}">
